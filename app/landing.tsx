...
import {Pause,Play,ArrowRight,Maximize2,Sparkles,Trash2,FolderOpen,RotateCcw,ShoppingBag,SkipForward} from "lucide-react";
...
export default function Landing({children,onPlay,onChoose,onEdit,model,designs=[],onOpenSaved,onContinueSaved,onShopSaved,onDeleteSaved,suspended=false}:{children:ReactNode;onPlay:()=>void;onChoose:(recipe:Recipe)=>void;onEdit:()=>void;model:BuildModel;designs?:SavedDesign[];onOpenSaved?:(design:SavedDesign)=>void;onContinueSaved?:(design:SavedDesign)=>void;onShopSaved?:(design:SavedDesign)=>void;onDeleteSaved?:(id:string)=>void;suspended?:boolean}){
 const [paused,setPaused]=useState(false),[fast,setFast]=useState(false),[done,setDone]=useState(false),[changing,setChanging]=useState(false),clock=useRef(new LandingAssemblyClock()),screen=useRef<HTMLDivElement>(null),choiceTimer=useRef<ReturnType<typeof setTimeout>|null>(null);
 useEffect(()=>{
 ...
 function skip(){clock.current.reduced=false;clock.current.paused=false;setPaused(false);clock.current.skip();setDone(true);}
 function toggleSpeed(){const next=clock.current.speed>1?1:2;clock.current.speed=next;setFast(next>1);}
 ...
 return <section className="creation-landing landing-with-film designs-first" aria-label="Explore brick designs">
  <div className={`landing-film ${changing?"is-changing":""}`} role="group" aria-label={`Assembly animation of ${model.name}`}>
   ...
   <div className="landing-film-caption"><div><span>{model.pieces.length.toLocaleString()} PIECES</span><h1><button type="button" className="landing-title-link" onClick={onEdit}>{model.name}</button></h1></div><div className="landing-film-controls">{!done&&<button className="film-speed" onClick={toggleSpeed} aria-pressed={fast} aria-label={fast?"Back to normal speed":"Play twice as fast"}>2×</button>}{!done&&<button onClick={skip} aria-label="Skip to the finished model"><SkipForward size={18}/></button>}<button onClick={toggle} aria-label={done?"Play the assembly again":paused?"Resume assembly":"Pause assembly"} aria-pressed={paused&&!done}>{done||paused?<Play size={18}/>:<Pause size={18}/>}</button><button onClick={replay} aria-label="Replay the assembly from the start"><RotateCcw size={18}/></button><button onClick={onPlay} aria-label="Open in the studio with the assembly playing"><Maximize2 size={18}/></button></div></div>
  </div>
