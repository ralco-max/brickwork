...
export default function Landing({children,onPlay,onChoose,onEdit,model,suspended=false}:{children:ReactNode;onPlay:()=>void;onChoose:(recipe:Recipe)=>void;onEdit:()=>void;model:BuildModel;suspended?:boolean}){
 ...
 return <section className="creation-landing landing-with-film designs-first" aria-label="Explore brick designs">
  ...
  <div className="design-quick-picks"><div role="group" aria-label="Preview a starting design">{([['bridge','Bridge'],['castle','Castle'],['lighthouse','Lighthouse'],['rocket','Rocket']] as const).map(([id,label])=><button key={id} aria-pressed={model.recipe===id} onClick={()=>choose(id)}>{label}</button>)}</div><button className="open-design" onClick={onEdit}>Open studio<ArrowRight size={15}/></button></div>
