...
import {reviewDesign,critiqueMassing} from "@/lib/design-review";
...
export async function POST(request:Request){
 ...
 return eventResponse(request,async(signal,send)=>{
  ...
  if(input.stage==="massing"){const critique=await critiqueMassing(input.brief,input.sheet as ReferenceSheet|undefined,input.signature||[],input.images,key,model,signal,provider.fetch,provider.recordUsage);send({type:"complete",critique});return;}
  const review=await reviewDesign(input.brief,input.images,key,model,signal,provider.fetch,input.revision,provider.recordUsage);
