...
const inputSchema=z.object({prompt:z.string().trim().min(3).max(2000),detail:z.enum(["small","medium","large"]),reference:z.string().max(1500000).regex(/^data:image\/jpeg;base64,[A-Za-z0-9+/=]+$/).optional(),research:referenceSheetSchema.optional(),previous:z.unknown().optional(),brief:briefSchema.optional(),locked:z.array(z.string().max(80)).max(240).optional(),feedback:z.array(z.string().max(1000)).max(40).optional(),history:z.array(z.object({request:z.string().max(2000),summary:z.string().max(1000)})).max(8).optional(),budget:z.object({limit:z.number().int().min(1).max(16000),used:z.number().int().min(0).max(100000),remaining:z.number().int().min(-100000).max(16000),byComponent:z.array(z.object({component:z.string().max(80),pieces:z.number().int().min(0).max(100000)})).max(241)}).optional()}).strict();
