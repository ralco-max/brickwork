...
const inputSchema=z.object({prompt:z.string().trim().min(3).max(2000),detail:z.enum(["small","medium","large"]),reference:z.string().max(1500000).regex(/^data:image\/jpeg;base64,[A-Za-z0-9+/=]+$/).optional(),research:z.object({searchable:z.boolean(),kind:z.string().max(20),subject:z.string().max(120),summary:z.string().max(300),length_m:z.number().nullable(),width_m:z.number().nullable(),height_m:z.number().nullable(),proportions:z.string().max(300),colors:z.array(z.string().max(200)).max(6),silhouette:z.array(z.string().max(200)).max(8),distinctive:z.array(z.string().max(200)).max(8),sources:z.array(z.string().max(200)).max(4),target:z.object({x:z.number().int().min(1).max(80),y:z.number().int().min(1).max(160),z:z.number().int().min(1).max(80),vertical:z.number().min(1).max(3)}).optional()}).optional(),previous:z.unknown().optional(),brief:briefSchema.optional(),locked:z.array(z.string().max(80)).max(240).optional(),feedback:z.array(z.string().max(1000)).max(40).optional(),history:z.array(z.object({request:z.string().max(2000),summary:z.string().max(1000)})).max(8).optional()}).strict();
