...
type Props={arrivals?:{epoch:number;paused:boolean};suspended?:boolean;pieces:Piece[];length:number;width?:number;height?:number;modelKey?:string;editMode?:string;onBrick?:(hit:BrickHit)=>void;highlightIds?:number[];explode:number;stage:number;selected:string|null;view:string;reset:number;rotate:boolean;onPick:(key:string|null)=>void;assembly?:{time:()=>number;onFrame?:(canvas:HTMLCanvasElement,time:number)=>void};onCanvas?:(canvas:HTMLCanvasElement|null)=>void};
export default function Viewport(props:Props){
 ...
 const [error,setError]=useState(false),[touchActive,setTouchActive]=useState(false);const touchEnabled=useRef(touchActive);touchEnabled.current=touchActive;
 ...
 useEffect(()=>{
  ...
  const cinematic=Boolean(props.assembly),presentation=cinematic||Boolean(props.arrivals),coarse=window.matchMedia("(pointer: coarse)").matches;
  ...
  setError(false);renderer.setPixelRatio(Math.min(window.devicePixelRatio,coarse?1.5:2));renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.setClearColor(presentation?0xffffff:0xeef2f4,presentation?1:0);if(renderer.domElement.parentElement!==node){node.appendChild(renderer.domElement);latest.current.onCanvas?.(renderer.domElement);}
  ...
  const controls=new OrbitControls(camera,renderer.domElement);controls.enabled=!cinematic;controls.enableDamping=!cinematic;controls.dampingFactor=.07;controls.minDistance=18;controls.maxDistance=Math.max(L,W,H)*6;controls.maxPolarAngle=Math.PI*.49;controls.target.set(0,H*.44,0);controls.autoRotateSpeed=.6;
  scene.add(new THREE.HemisphereLight(0xffffff,0x8794a2,2.8));const sun=new THREE.DirectionalLight(0xfff6e6,3.3);sun.position.set(-20,80,50);sun.castShadow=true;sun.shadow.mapSize.set(coarse?1024:2048,coarse?1024:2048);sun.shadow.camera.left=-75;sun.shadow.camera.right=75;sun.shadow.camera.top=70;sun.shadow.camera.bottom=-70;sun.shadow.bias=-.0004;sun.shadow.normalBias=.06;scene.add(sun);
  ...
  const maxStage=Math.max(1,...props.pieces.map(p=>p.stage));
  ...
  const tracks=cinematic?assemblyTracks(props.pieces,L,W,props.height??43):null;
  const groups=new Map<string,Piece[]>();for(const p of props.pieces){const k=`${p.part}:${p.color}:${p.rotated}`;const list=groups.get(k)||[];list.push(p);groups.set(k,list);}
  const batches:{body:THREE.InstancedMesh;studs:THREE.InstancedMesh|null;pieces:Piece[]}[]=[],meshes:THREE.InstancedMesh[]=[];
  ...
  const update=(state:Props,time?:number)=>{
   controls.enabled=!cinematic&&(!coarse||touchEnabled.current);renderer.domElement.style.touchAction=coarse&&!touchEnabled.current?"pan-y":"none";
   ...
   for(const batch of batches){let studIndex=0;batch.pieces.forEach((p,i)=>{
    ...
    const y=p.y*.4+state.explode*(p.stage/maxStage*Math.max(10,H*.45)+p.y*.05);
    ...
    if(!animationOnly){baseColor.set(COLORS[p.color].hex);if(highlighted.size){if(highlighted.has(p.id))baseColor.set("#fb541e");else baseColor.lerp(dimColor,.85);}else if(!selected)baseColor.lerp(dimColor,.85);batch.body.setColorAt(i,baseColor);}
    if(batch.studs)for(let x=0;x<p.w;x++)for(let z=0;z<p.d;z++){studLocal.makeTranslation(x+.5-p.w/2,p.h*.2+.07,z+.5-p.d/2);studWorld.multiplyMatrices(temp.matrix,studLocal);batch.studs.setMatrixAt(studIndex,studWorld);if(!animationOnly)batch.studs.setColorAt(studIndex,baseColor);studIndex++;}
  ...
  const setCamera=(view:string)=>{
   ...
   const displayH=H+latest.current.explode*(Math.max(10,H*.45)+H*.125);
   ...
   let distance=1;
   for(const x of [-L/2,L/2])for(const y of [0,displayH])for(const z of [-W/2,W/2]){const corner=new THREE.Vector3(x,y,z).sub(target);distance=Math.max(distance,corner.dot(direction)+Math.max(Math.abs(corner.dot(right))/Math.tan(hFov/2),Math.abs(corner.dot(up))/Math.tan(vFov/2)));}
  ...
  const resize=()=>{cameraRevision++;const w=node.clientWidth,h=node.clientHeight;renderer.setSize(w,h);camera.aspect=w/Math.max(h,1);camera.updateProjectionMatrix();if(cinematic){setCamera("perspective");camera.position.sub(controls.target).multiplyScalar(1.4).add(controls.target);cinematicCamera.copy(camera.position).sub(controls.target);}};
  const observer=new ResizeObserver(resize);observer.observe(node);resize();if(savedCamera.current?.key===(props.modelKey??"model")){camera.position.copy(savedCamera.current.position);controls.target.copy(savedCamera.current.target);controls.update();}else setCamera(latest.current.view);update(latest.current);api.current={update,camera:(view)=>{if(cinematic||reducedMotion.matches){cameraMove=null;setCamera(view);return;}const position=camera.position.clone(),target=controls.target.clone();setCamera(view);cameraMove={position,target,toPosition:camera.position.clone(),toTarget:controls.target.clone(),start:performance.now()};camera.position.copy(position);controls.target.copy(target);camera.lookAt(target);}};
...
const time=latest.current.assembly?.time();if(time!==undefined&&(time!==lastTime||cameraRevision!==lastCameraRevision)){update(latest.current,time);camera.position.copy(cinematicCamera).applyAxisAngle(axis,(time-.5)*.38).multiplyScalar(1-.08*time).add(controls.target);camera.lookAt(controls.target);lastTime=time;lastCameraRevision=cameraRevision;}controls.update();renderer.render(scene,camera);if(time!==undefined)latest.current.assembly?.onFrame?.(renderer.domElement,time);}};animate();
 ...
 useEffect(()=>{api.current?.update(props);},[props.explode,props.stage,props.selected,props.rotate,props.highlightIds,touchActive]);
 useEffect(()=>{api.current?.camera(props.view);},[props.view,props.reset,props.explode]);
 return <div className={"canvas-host "+(props.editMode&&props.editMode!=="inspect"?"is-editing":"")} ref={host}>{!props.assembly&&!error&&<button className="touch-model-toggle" aria-pressed={touchActive} onClick={()=>setTouchActive(v=>!v)}>{touchActive?"Done · scroll page":"Touch to rotate or edit"}</button>}{error&&<div className="canvas-error"><strong>3D preview is unavailable in this browser.</strong><p>The parts audit and exports still work. Try opening the studio in Safari or Chrome with WebGL enabled.</p></div>}</div>;
}
