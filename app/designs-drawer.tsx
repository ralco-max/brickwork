"use client";
import {FolderOpen,Sparkles,ShoppingBag,Trash2,Landmark,X} from "lucide-react";
...
type Props={open:boolean;onClose:()=>void;model:BuildModel;designs:SavedDesign[];onChoose:(recipe:Recipe)=>void;onOpenSaved:(design:SavedDesign)=>void;onContinueSaved:(design:SavedDesign)=>void;onShopSaved:(design:SavedDesign)=>void;onDeleteSaved:(id:string)=>void};
export default function DesignsDrawer({open,onClose,model,designs,onChoose,onOpenSaved,onContinueSaved,onShopSaved,onDeleteSaved}:Props){
 return <aside className="designs-drawer" aria-label="Designs" aria-hidden={!open} inert={!open}><div className="designs-drawer-inner">
  <div className="your-designs-head drawer-head"><span>LANDMARKS</span><small>Rebuilt from real blueprints</small><button type="button" className="tools-close drawer-close" aria-label="Close designs" onClick={onClose}><X size={18}/></button></div>
