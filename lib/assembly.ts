...
// a dialog must not skip the film on return.
export class LandingAssemblyClock{
 ...
 tick(now:number){const delta=this.last===null?0:Math.max(0,Math.min(.08,(now-this.last)/1000));this.last=now;if(!this.paused&&!this.reduced)this.elapsed=(this.elapsed+delta)%24;if(this.reduced||this.elapsed<2)return 1;if(this.elapsed<4){const u=(this.elapsed-2)/2;return 1-u*u*(3-2*u);}return Math.min(1,(this.elapsed-4)/18);}
