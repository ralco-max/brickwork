...
// How far the bricks start from home: a little spread sideways and well above the frame, so
// the intro reads as pieces raining in from the sky rather than a cloud hovering in view.
export function assemblyReach(length:number,width:number,height:number){return {distance:Math.max(10,Math.max(length,width)*.3),lift:Math.max(70,height*.4*1.6)};}
export function assemblyTracks(pieces:Piece[],length:number,width:number,height:number){
 ...
 sorted.forEach((p,rank)=>{const seed=p.id+1,angle=hash(seed)*Math.PI*2;tracks.set(p.id,{start:.025+.70*rank/Math.max(1,sorted.length-1),angle,distance:reach.distance*(.5+hash(seed+9)*.9),lift:reach.lift+hash(seed+3)*50,rx:(hash(seed+5)-.5)*1.5,ry:(hash(seed+7)-.5)*2.6,rz:(hash(seed+11)-.5)*1.3});});return tracks;
}
