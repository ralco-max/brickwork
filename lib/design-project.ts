...
const manualBrick=boxSchema.extend({id:z.number().int(),part:z.string(),color:z.string(),rotated:z.boolean(),stage:z.number().int(),face:z.enum(["px","nx","pz","nz"]).optional(),support:z.boolean().optional()}).refine(p=>Object.hasOwn(PARTS,p.part)&&Object.hasOwn(COLORS,p.color)&&p.x+p.w<=192&&p.y+p.h<=240&&p.z+p.d<=192).refine(p=>{const t=PARTS[p.part as keyof typeof PARTS];return !!t&&p.w===(p.rotated?t.d:t.w)&&p.d===(p.rotated?t.w:t.d)&&p.h===t.h;});
