...
export async function researchSubject(idea:string,key:string,model:string,signal:AbortSignal,fetcher:typeof fetch=fetch,onUsage?:(usage:unknown,extra:{searchCalls:number})=>Promise<void>):Promise<ReferenceSheet&{thinking?:string;searches?:string[]}>{
 ...
 return {thinking,searches,searchable:raw.searchable===true,kind:(kinds as readonly string[]).includes(String(raw.kind))?raw.kind as SubjectKind:"object",subject:String(raw.subject||idea).slice(0,120),summary:String(raw.summary||"").slice(0,300),length_m:num(raw.length_m),width_m:num(raw.width_m),height_m:num(raw.height_m),proportions:String(raw.proportions||"").slice(0,300),colors:list(raw.colors,6),silhouette:list(raw.silhouette,8),distinctive:list(raw.distinctive,8),sources:list(raw.sources,4).filter(u=>/^https:\/\//.test(u))};
}
