...
export function budgetedProvider(db:BudgetDatabase,key:string,notify:(budget:BudgetSnapshot)=>void,fetcher:typeof fetch=fetch){
 let reservation:{id:string;input:number;maxOutput:number}|undefined;
 ...
 const guardedFetch:typeof fetch=async(url,init)=>{
  ...
  if(body.model!==BUDGET_MODEL||!Number.isSafeInteger(body.max_output_tokens)||body.max_output_tokens<1||body.max_output_tokens>32000||body.tools)throw new GenerationError("The spending limit needs verified pricing for this model. No AI request was started.","BUDGET_MODEL");
  ...
  const input=counted.input_tokens+new TextEncoder().encode(JSON.stringify(body.text??{})).length+4096;
 ...
 const recordUsage=async(value:unknown)=>{
  if(!reservation||!value||typeof value!=="object")return;
  const usage=value as Usage,cached=usage.input_tokens_details?.cached_tokens??0;
  let amount:number;
  try{amount=tokenCost(usage.input_tokens,usage.output_tokens,cached);}catch{return;}
  ...
  if(usage.input_tokens>reservation.input||usage.output_tokens>reservation.maxOutput){
