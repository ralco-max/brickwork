...
1. Describe the idea and choose **Main subject** or **Whole site**. The optional composition note and required features can specify a version, orientation, included structures and exclusions.
2. Research creates one reusable reference pack: cited sources, up to two relevant photos/plans selected from actual image-search results, component names, spatial relationships, uncertainty and proportions. An uploaded photo accompanies the pack. Real subjects without usable sources or a supplied photo stop here, before generation. Generic subjects can proceed without factual measurements.
3. One streamed design call creates the complete scene. The browser compiles its shapes into catalog bricks and checks the actual finished model, with the selected smoothing, hollowing and manual edits. There is no mandatory blockout, blockout critique or automatic trimming call.
4. One review compares the actual bricks against the same saved references and photo, with front, perspective, rear, end and overhead model views. Composition mismatch or uncertainty prevents a successful review. Failed digital checks remain visible; a targeted revision is an explicit user action.
5. References survive browser saving, project export/import and reopening. A revision reuses them when the subject, scope, composition, required features and uploaded photo are unchanged. A changed brief or an older project without references gets a new lookup. The size guide preserves proportions and leaves space for the base; it does not automatically exaggerate height or force a generic subject to fill its bounding box.

A normal fresh design is three paid Responses calls: research, design, review. A revision with a reusable reference is two; Review again is one. Each request retains the existing input-token preflight and $10 ledger guard. The UI reports this run's recorded usage cost separately from remaining budget; unresolved requests may still have budget held. No fixed per-design price is advertised.

Use a personal OpenAI key or the configured studio key. Personal keys remain in tab memory, pass through the same-origin server to OpenAI and are never exported. Reference photos are now saved locally with the design and included in its portable project export. Generation uses a 32,000 output-token ceiling; a review uses 4,000. All requests use `store: false`.
...
The intro is a settle. The model is there from the first frame, but loosened: every brick waits a short way from its spot, nudged sideways and up and turned a little, like the finished model shaken apart. Then it tightens bottom-up, each brick sliding the last short distance and seating with a small overshoot, so the silhouette reads from second one and the last bricks feel like a finish rather than a trickle. Explode is the intro run backwards: the slider scrubs the whole assembly timeline from the finished model to its first frame, so the last bricks to land are the first to lift off, each flying back out along its own path to where it waited in the air, and the camera pulls back to keep the cloud in frame.
...
The design instructions carry guidance from experienced brick sculptors: brick math (2 studs of width equal 5 plates of height), footprint first then masses then texture, silhouette and proportion over fuss, an analogous palette with one complementary accent at the focal point, micro-texture with purpose, staggered courses and tied corners, returns for overhangs, an internal solid core in large volumes, finished bases and deliberate negative space. The visual reviewer judges with the same eye and asks for specific, buildable improvements. Designs are judged in the round: the reviewer sees five views (front, front-right, rear-left, the left end and overhead), and the instructions require every face to carry its real features, so a side profile extruded across its width with blank ends is called out as not recognizable.
...
`lib/design-research.ts` creates a shared composition with named included/excluded components and relations such as left_of, behind or above. Relation endpoints must name included components and cannot contradict exclusions. The source list comes from the search tool's returned sources and citations; selected image URLs must match actual image results. Unknown dimensions stay null and uncertain relationships stay explicit. A source-backed lookup is evidence, not a guarantee every inferred architectural fact is correct.

The designer and reviewer receive that same sheet and its source images, plus any uploaded photo. An airport's full-site scope must retain its requested airfield; terminal-only scope can omit it. There is no Dulles-specific rule that silently changes the scope. The overhead review image is oriented with x right and +z down.

`targetExtents` gives a uniformly scaled suggestion using metres, studs and plates. It reserves space for the base. Generic subjects receive no invented binding dimensions. Component cost feedback uses the actual packed model, so its total includes finish and manual edits. Catalog packing still favors large parts and connections; estimated parts prices are not live shopping quotes.
...
The catalog has three real wheel elements, rim and tire: 18 mm (3 studs across), 30.4 mm (4) and 43.2 mm (6). A `wheel` shape gives a centre, a diameter and the axle axis, and the compiler places the nearest element as one piece, clearing whatever the scene drew inside it. Wheels have no studs; the audit attaches a wheel to whatever piece meets its hub along the axle, so the instructions ask for a black axle box through each wheel centre and an arch carved out of the body. The viewport and the review renderer draw the tire as a cylinder on the axle with a light gray hub, and the parts list and BrickLink export carry the element ids.

## Worked examples

`lib/design-examples.ts` holds four finished scenes in the wire format, each a construction done the way it packs and reads well: a hatchback with four wheel elements on axle boxes and a raked windscreen lean; a cottage whose pitched roof is stepped courses with overhanging eaves; an open lattice tower of leg and brace leans with a core under its platforms; and a standing dog with planted legs, a muzzle, ears and a lean for the tail. Two are chosen per brief by the reference sheet's kind and the words in the idea and sent with every design or revision call, and the instructions say to match their specificity, not their subject. A test compiles all four and checks they pack clean.

## Leaning members

Every floating piece in the Eiffel Tower runs came from the same thing: a splayed leg or a brace drawn as a thin box or a beam at an angle, which the voxelizer turns into single cells stepping sideways with nothing under them. The scene format now has a `lean` kind for exactly those members. The designer gives the centre of its bottom end, the centre of its top end, its width across in studs and its course height (3 plates for bricks, 1 for plates), and `leanCourses` in `lib/generated-scene.ts` lays it as stepped courses: each course is a box along the direction of the lean, long enough that it overlaps the course below by at least a stud (two when the lean runs diagonally in plan), centred on the line at that height. A leg built this way stands on its own, packs into ordinary bricks and passes the support and clutch checks. The instructions require every splayed leg, brace, buttress, strut, sloping roof edge, cable stay, leaning wall and tapering tower edge to be a lean, and keep beams for round members that are horizontal or vertical.

## Slopes

Studs-up bricks alone make every curve a staircase, which is why a bean looked like a bunker. Before the packer runs, `carveSlopes` in `lib/slopes.ts` looks for the staircase patterns a voxelized curve or pitch leaves behind and swaps each for the slope part that matches its rise and run: a 3-plate step with the staircase rising behind it becomes a 45° slope (2 × 1, or the 2 × 2 with its studded back row when the column behind is solid), a 2-plate step a cheese slope (1 × 1 or 1 × 2), two 1-plate steps over two studs a curved slope (1 × 2 or 2 × 2), a 3-plate rise over two studs a 33° slope (3 × 1 or 3 × 2), and the same step on an underside an inverted 45° slope held by the course above. A slope is fitted only where the surface keeps rising behind it, so a flat roof edge stays square, only where it rests on something, and only where the column directly behind the step is solid, because on a one-stud curved skin carving the step just fragments the skin into small pieces with less clutch; an inverted pair is refused when it would strand a neighbouring overhang cell that needs those cells to bridge back into the mass. The cells a slope takes leave the voxel map and the packer fills the rest, so coverage and overlap checks still hold. Each slope piece carries a `face` (the direction its slope descends) and `rotated` follows from it, so project files, the design library, manual edits and the LDraw export (orientation approximate for slopes) all round-trip. Stud contact, in the audit, the clutch simulation and the renderers, comes from `studCells`: bricks and plates everywhere, tiles nowhere, studded slopes only on their flat back row, inverted slopes across their top. The viewport and the review renderer build slope geometry from `lib/brick-geometry.ts`, so the visual critic sees the same wedges the viewer does. Slopes add pieces (roughly 15 to 30 percent on a curved surface); the final packed count includes that increase. Landmark presets are packed without the slope pass so their fixtures stay as designed.

## Generation flow

`lib/design-workflow.ts` owns the order: lookup (or reuse), one design, local compile, one review. A failed lookup/design stops later calls; a failed review keeps the completed model. Over-budget designs remain flagged drafts without automatic paid retries. Refinement sends current checks, review improvements and actual component counts; unchanged geometry remains a diff.

The Designer choice still selects Terra, Sol or Astra for geometry, with Terra for research and review. Existing pricing and atomic per-key reservations remain enforced. Recorded per-request cost is calculated from provider-reported tokens and actual search calls; budget holds are not reported as confirmed spend.
...
The AI JSON schema now carries the same string lengths, integer dimensions, coordinate ranges and radius bounds as the local validator. Geometry errors remain visible for a user-requested targeted repair. Service and account failures require a user retry and never silently trigger extra paid requests.

A successfully compiled streaming snapshot with subject geometry can be kept after interruption or Stop. A foundation alone is not presented as an unfinished custom design. Locked components are validated before a snapshot becomes recoverable. The recovery action is visible beside the canvas and continues with the original brief and the saved scene. Incomplete drafts are marked unfinished and cannot pass the final review gate. Kept partial drafts save their unfinished state and outstanding revision request in the browser design library and project export, so reopening can continue the same task.
...
Opening the tools does not change pages. The tools panel slides in on the right, the drawer folds away to make room, and the 3D view stays exactly where it was, camera included. The model's name, piece count, build details and the Refine with AI and Edit bricks actions sit on the stage itself as a HUD in the top left, mirroring the playback and camera controls on the right, so nothing stacks above the view: the assembly intro only plays when a new model arrives, never because a panel opened. With the tools open the stage gains the brick editor, the model metrics and the Parts audit and Build guide sections below it, and the settings panel (size, colors, dimensions, estimated cost) beside it. Clicking the model while the tools are closed opens them; a drag still orbits. The slow camera sweep during the intro runs only while the tools are closed, so the stage reads as a presentation until you start working. Both panels can be reopened at any time; below 1240px an open drawer floats over the stage while the tools are open. On a phone the command bar is a single row (drawer and Make it yours as icons around the idea field), the drawer opens above the stage, and the tools panel is a bottom sheet that slides over the stage, scrolls on its own and has its own close button, so the model stays in reach while you adjust it.


## Personal AI budget

A fixed $10 USD budget applies to Brickwork token usage per API key from this version onward. It includes generation, visual reviews, every manual revision and retry. It does not reset on refresh, closing the dialog or disconnecting/reconnecting the same key. It does not include earlier calls, usage outside this Site, taxes or custom account pricing. The key remains in tab memory. D1 stores only a SHA-256 fingerprint, timestamps and monetary ledger entries, never the raw key, prompt or images.
...
`node --test tests/reference-workflow.test.mjs tests/design-workflow.test.mjs tests/design-library.test.mjs tests/generation-resilience.test.mjs tests/ai-budget.test.mjs tests/slopes.test.mjs` exercises ordered call counts, over-budget drafts without automatic trimming, failed lookup/cancellation, source provenance, source-image forwarding, composition rejection, saved references and actual component-count parity. These are authored fixtures and mocked provider responses.

Use `tests/accuracy-benchmarks.json` and `docs/accuracy-evaluation.md` for live comparisons against baseline commit `10079cb908663ddb601065f03ef6b874954baf27`. Live image-search compatibility, image URL availability, generated visual fidelity and real cost distributions require a connected API key; the automated tests do not establish those outcomes.
