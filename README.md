...
`lib/design-examples.ts` holds four finished scenes in the wire format, each a construction done the way it packs and reads well: a hatchback with four wheel elements on axle boxes and a raked windscreen lean; a cottage whose pitched roof is stepped courses with overhanging eaves; an open lattice tower of leg and brace leans with a core under its platforms; and a standing dog with planted legs, a muzzle, ears and a lean for the tail. Two are chosen per brief by the reference sheet's kind and the words in the idea and sent with every design and detail call (not the massing pass), and the instructions say to match their specificity, not their subject. A test compiles all four and checks they pack clean.
