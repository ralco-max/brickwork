...
Explode blows the model apart like a rubber-band ball: every brick moves straight out from the model's centre, in every direction, to up to 3.4 times its distance, tumbling on its intro spin; the expansion is about the centre of the base so the bottom row spreads outward and everything else lifts and spreads, and the camera pulls back to keep the whole cloud in frame.
