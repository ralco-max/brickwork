...
Every brick is in the air from the first frame of the intro, waiting at the far end of its own path: behind the model at a modest height with a little sideways spread. Each one swooshes forward and up over the build, crests, and settles into place, so the late arrivals are already on their way instead of appearing out of nowhere. Explode is the intro run backwards: the slider scrubs the whole assembly timeline from the finished model to its first frame, so the last bricks to land are the first to lift off, each flying back out along its own path to where it waited in the air, and the camera pulls back to keep the cloud in frame.
